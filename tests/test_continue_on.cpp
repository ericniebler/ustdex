...
TEST_CASE("continue_on calls the receiver when the scheduler dictates", "[adaptors][continue_on]")
{
  ...
  auto snd = ex::then(ex::continue_on(ex::just(13), sched), [&](int val) {
    called = true;
    return val;
  });
...
TEST_CASE("continue_on calls the given sender when the scheduler dictates", "[adaptors][continue_on]")
{
  ...
  impulse_scheduler sched;
  auto snd = ex::then(ex::continue_on(std::move(snd_base), sched), [&](int val) {
    ++counter;
    return val;
  });
          ...
          "[adaptors][continue_on]")
{
  ...
  check_error_types<std::exception_ptr>(ex::continue_on(ex::just(potentially_throwing{}), sched));
}
