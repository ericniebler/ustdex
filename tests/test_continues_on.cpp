...
TEST_CASE("continues_on calls the receiver when the scheduler dictates", "[adaptors][continues_on]")
{
  bool called{false};
...
TEST_CASE("continues_on calls the given sender when the scheduler dictates", "[adaptors][continues_on]")
{
  int counter{0};
...
TEST_CASE("continues_on has the values_type corresponding to the given values", "[adaptors][continues_on]")
{
  inline_scheduler sched{};

  check_value_types<_m_list<int>>(ex::continues_on(ex::just(1), sched));
  check_value_types<_m_list<int, double>>(ex::continues_on(ex::just(3, 0.14), sched));
  check_value_types<_m_list<int, double, std::string>>(ex::continues_on(ex::just(3, 0.14, std::string{"pi"}), sched));
}
...
TEST_CASE("continues_on keeps error_types from scheduler's sender", "[adaptors][continues_on]")
{
  ...
  check_error_types<std::exception_ptr>(ex::continues_on(ex::just(2), sched2));
  check_error_types<int>(ex::continues_on(ex::just(3), sched3));
}

TEST_CASE("continues_on sends an exception_ptr if value types are potentially throwing when copied",
...
//   auto snd = ex::on(sched_A, ex::just() | ex::continues_on(sched_B));
//   std::string res;
