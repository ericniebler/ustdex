...
namespace ustdex {
  ...
  namespace _stok {
    struct _inplace_stop_callback_base {
     ...
     protected:
      ...
      explicit _inplace_stop_callback_base( //
      ...
      std::atomic<bool> _callback_completed{false};
    };
  ...
  struct never_stop_token {
   private:
    struct _callback_type {
      explicit _callback_type(never_stop_token, _ignore) noexcept {
      }
    };
  ...
  class inplace_stop_source {
   public:
    ...
    auto stop_requested() const noexcept -> bool {
      return (_state.load(std::memory_order_acquire) & _stop_requested_flag) != 0;
    }
   ...
   private:
    ...
    void _unlock(uint8_t) const noexcept;
  ...
  class inplace_stop_token {
   ...
   private:
    ...
    explicit inplace_stop_token(const inplace_stop_source *_source) noexcept
      : _source(_source) {
    }
  ...
  class inplace_stop_callback : _stok::_inplace_stop_callback_base {
   public:
    ...
    explicit inplace_stop_callback(inplace_stop_token _token, _Fun2 &&_fun) noexcept(
  ...
  inline inplace_stop_source::~inplace_stop_source() {
    USTDEX_ASSERT((_state.load(std::memory_order_relaxed) & _locked_flag) == 0);
  ...
  inline auto inplace_stop_source::request_stop() noexcept -> bool {
    ...
    while (_callbacks != nullptr) {
      ...
      _state.store(_stop_requested_flag, std::memory_order_release);
      ...
      if (!_removed_during_callback) {
        ...
        _callbk->_callback_completed.store(true, std::memory_order_release);
      }
  ...
  inline auto inplace_stop_source::_lock() const noexcept -> uint8_t {
    ...
    auto _old_state = _state.load(std::memory_order_relaxed);
    do {
      while ((_old_state & _locked_flag) != 0) {
        _spin._wait();
        _old_state = _state.load(std::memory_order_relaxed);
      }
  ...
  inline void inplace_stop_source::_unlock(uint8_t _old_state) const noexcept {
    (void) _state.store(_old_state, std::memory_order_release);
  }
    ...
    if (_callbk->_prev_ptr != nullptr) {
      ...
      *_callbk->_prev_ptr = _callbk->_next;
      ...
      if (std::this_thread::get_id() == _notifying_thread) {
        ...
        _stok::_spin_wait _spin;
        while (!_callbk->_callback_completed.load(std::memory_order_acquire)) {
          _spin._wait();
        }
      }
    }
