...
namespace ustdex {
  ...
  namespace _when_all {
    ...
    template <class>
    struct _and_nothrow_copyable;

    template <>
    struct _and_nothrow_copyable<_mtrue> {
      template <class... Ts>
      using _f = _mbool<_nothrow_decay_copyable<Ts...>>;
    };

    template <>
    struct _and_nothrow_copyable<_mfalse> {
      template <class... Ts>
      using _f = _mfalse;
    };
    ...
    // NVIDIA's compilers do not like this formulation:
    // template <class Offsets, class... Sigs>
    // auto operator+(_completion_metadata<_mtrue, _mtrue, Offsets, Sigs...> &)
    //   -> _mpair<_concat_completion_signatures<completion_signatures<Sigs...>>, Offsets>;

    // template <class ValsOK, class ErrsOK, class Offsets, class... Sigs>
    // auto operator+(_completion_metadata<ValsOK, ErrsOK, Offsets, Sigs...> &) //
    //   -> _mpair<                                                             //
    //     _concat_completion_signatures<
    //       completion_signatures<Sigs..., set_error_t(std::exception_ptr)>>,
    //     Offsets>;
    ...
    auto operator|(
      ...
      -> _completion_metadata<
        ValsOK,
        _minvoke<_and_nothrow_copyable<ErrsOK>, As...>,
        _moffsets<>,
        Sigs...,
        Tag(_decay_t<As>...)> &;
    ...
    auto operator|(
      ...
      set_value_t (*)(As...))
      -> _completion_metadata<
        _minvoke<_and_nothrow_copyable<ValsOK>, As...>,
        ErrsOK,
        _moffsets<>,
        set_value_t(_decay_t<As>...), // Insert the value signature at the front
        Sigs...> &;

    // This overload is selected when we see the second set_value_t
    // completion signature. Senders passed to when_all are only allowed
    // one set_value completion.
    template <class ValsOK, class ErrsOK, class... As, class... Sigs, class... Bs>
    auto operator|(
      _completion_metadata<ValsOK, ErrsOK, _moffsets<>, set_value_t(As...), Sigs...> &,
      set_value_t (*)(Bs...)) -> _tombstone &;

    // Propagate the error
    auto operator|(_tombstone &, _ignore_t) -> _tombstone &;
    ...
    // This overload causes an error to be propagated.
    ...
    // The following two overloads are selected when one of the metadata structs
    // is for a sender with no value completions. In that case, the when_all can
    // never complete successfully, so drop the set_value completion signature.
    template <
      class LeftValsOK,
      class LeftErrsOK,
      class Offsets,
    ...
    template <class... What>
    ERROR<What...> &operator&(ERROR<What...> &, _ignore_t);
  ...
  struct when_all_t {
   ...
   private:
    ...
    template <class Rcvr, class Errors, class Values, class StopToken>
    struct _data_t;

    template <class Data>
    struct _env_t;

    template <class Data, std::size_t Offset>
    struct _rcvr_t;

    template <class CvFn, class Sndrs, class Rcvr>
    struct _opstate_t;
   ...
   public:
    template <class... Sndrs>
    USTDEX_HOST_DEVICE auto operator()(Sndrs... sndrs) const;
  };
  ...
  template <class Rcvr, class Values, class Errors, class StopToken>
  struct when_all_t::_env_t<when_all_t::_data_t<Rcvr, Values, Errors, StopToken>> {
  ...
  struct when_all_t::_rcvr_t<when_all_t::_data_t<Rcvr, Values, Errors, StopToken>, Offset> {
    using receiver_concept = receiver_t;

    using _data_t = when_all_t::_data_t<Rcvr, Values, Errors, StopToken>;
    _data_t *_data;
      ...
      if (_error != _data->_state.exchange(_error)) {
        ...
        if constexpr (_nothrow_decay_copyable<Error>) {
          ...
          try {
            ...
            _data->_errors.template emplace<std::exception_ptr>(std::current_exception());
          }
        }
      }
    }
  ...
  struct when_all_t::_connect_subs_fn {
  ...
  struct when_all_t::_opstate_t<CvFn, _tupl<std::index_sequence<Idx...>, Sndrs...>, Rcvr>
    : _immovable {
    ...
    using _completions_offsets_pair_t = //
      _when_all::_collect_outer<
        _when_all::_inner_completions<_minvoke1<CvFn, Sndrs>, _env_t>...>;
    ...
    using _data_t = when_all_t::_data_t<Rcvr, _values_t, _errors_t, _stok_t>;
  ...
  template <class... Sndrs>
  USTDEX_HOST_DEVICE auto when_all_t::operator()(Sndrs... sndrs) const {
    return _sndr_t<Sndrs...>{{}, {}, {static_cast<Sndrs &&>(sndrs)...}};
  }
